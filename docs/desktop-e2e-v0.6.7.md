...
- Clean Quit: relaunched the production bundle with unchanged welcome content, raised its window and selected the native Quit menu. A subsequent process check confirmed that production bundle's process had ended. Pending: eventual published-install retesting. Dock/system termination is not covered by the native menu/Command-Q result. The repair release is being prepared as v0.6.8; it has not yet been published or installed.
