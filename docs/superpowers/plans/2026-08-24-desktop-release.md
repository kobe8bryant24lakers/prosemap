...
- Keep documents local and store provider credentials in the operating system credential store.
...
These preview packages are not notarized by Apple and are not signed with a commercial Windows certificate.
...
Compare each package with `SHA256SUMS.txt` before opening it.
